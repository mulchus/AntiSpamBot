...
def menu(message):
    if admin_check(message):
        if config.mat: # если словарь мата не пустой (признак, что бот ранее стартовал нормально)
            ...
            info_message = bot.send_message(message.chat.id, f"Сначала надо запустить бота \n/start")
            time.sleep(config.pause)
            del_bot_mes(message.chat.id, message.message_id, info_message.message_id)
            ...
            for bot_settings in config.bot_settings:
                if bot_settings["chat_id"] == call.message.chat.id:
                    bot.send_message(call.message.chat.id,
                                     f'Запрещенные сообщения: {bot_settings["forbidden_message"]}',
...
def admin_check(message): # проверка написавшего на админа
    global admins
